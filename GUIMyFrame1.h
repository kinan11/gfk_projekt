...
public:
	...
	static void DisplayPic(wxPanel* parent, wxString path, wxScrolledWindow* display, wxFlexGridSizer* fgSizer);
	static void DisplayMetaData(wxGrid* EXIF, wxGrid* IPTC, wxPanel* parent, wxScrolledWindow* display, wxString path);
	static void DisplayFolder(wxPanel* parent, wxScrolledWindow* display, wxString& path, wxBoxSizer* sizer, wxStaticBitmap* sb);
	//FreeImage
...
class Panel2 : public wxScrolledWindow {
	friend class MyButton;
	friend class GUIMyFrame1;
	friend class MyCanvas;
public:
	Panel2() = default;
	Panel2(wxPanel* parent, wxString path, wxScrolledWindow* display, wxFlexGridSizer* fgSizer, const wxSize& pos = wxDefaultSize) :wxScrolledWindow(display, -1, wxPoint(0, 0), pos), _parent(parent), _path(path), _DisplayPanel(display), _fgSizer(fgSizer)
	{
...
private:
	wxPanel* _parent;
	wxString _path;
	wxScrolledWindow* _DisplayPanel;
	wxFlexGridSizer* _fgSizer;
	wxBoxSizer* sizer;
	wxStaticBitmap* sb;
	void OnPanelDoubleDown(wxMouseEvent& event)
	{
		GUIMyFrame1::DisplayFolder(_parent, _DisplayPanel, _path,sizer,sb);
	}
	void PaintFD()
	{
		...
		/*_DisplayPanel->SetScrollRate(16, 16);
		sizer = new wxBoxSizer(wxHORIZONTAL);
		//delete(sb);
		sb = new wxStaticBitmap(_DisplayPanel, wxID_ANY, b1);
		sizer->Add(sb, 0, wxALIGN_TOP, 0);
		_DisplayPanel->SetSizer(sizer);
		sizer->FitInside(_DisplayPanel);*/
		wxClientDC dc(this);
		dc.Clear();
		if (w_i > h_i)
		{
...
public:
	...
	MyButton(wxFlexGridSizer* fgSizer1, wxPanel* parent, wxScrolledWindow* displaypanel, wxGrid* EXIF, wxGrid* IPTC, wxWindowID id, const wxBitmap& bitmap, const wxPoint& pos = wxDefaultPosition, wxString path_array = 'a') : wxBitmapButton(parent, -1, bitmap, pos)
	{
